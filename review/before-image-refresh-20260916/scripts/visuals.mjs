...
const img=(name,alt,cls='')=>`<img class="${cls}" src="/assets/${name}.webp" alt="${e(alt)}" loading="lazy" width="960" height="640">`;
...
export function infographic(kind='culture'){
 ...
 const items=flows[kind];return `<div class="infographic infographic-${kind}" role="group" aria-label="${kind==='culture'?'복합배양 기술의 흐름':'단계별 연결'}">${items.map(([ic,t,d],i)=>`<div class="info-node ${t==='GMK®'?'info-core':''}"><span class="info-num">0${i+1}</span><div class="info-symbol">${icon(ic)}</div><h3>${t}</h3><p>${d}</p>${i<items.length-1?'<span class="info-arrow" aria-hidden="true">→</span>':''}</div>`).join('')}</div>`;
}
const iconsById={C03:['culture','search','partners'],R02:['culture','shield','flask'],I02:['culture','document','search','box'],I03:['network','box','partners'],RP02:['culture','leaf','book']};
function dashCards(ts,icons=[],className=''){
 return `<div class="concept-cards ${className}">${ts.map((t,i)=>{const parts=t.split(' — ');return `<article class="concept-card">${label('0'+(i+1))}${icon(icons[i%icons.length]||'network')}<h3>${e(parts[0])}</h3>${parts[1]?text(parts.slice(1).join(' — ')):''}</article>`}).join('')}</div>`;
}
...
export function enhanceSection(id,blocks,render){
 ...
 if(id==='C02')return `<div class="editorial-split"><div>${label('SINCE 2015')}<h3 class="editorial-title">${e(ts[0])}</h3>${ts.slice(1).map(text).join('')}<div class="mini-signature"><strong>GIUNCHAN</strong><span>Nature · Science · Everyday</span></div></div>${photo('company-nature','자연에서 시작된 소재의 가능성')}</div>`;
 if(id==='C03')return `<div class="concept-cards company-focus">${ts.map((t,i)=>{const [title,...desc]=t.split(' — ');return `<article class="concept-card">${img(['company-culture','company-research','company-partnership'][i],title)}<h3>${e(title)}</h3>${text(desc.join(' — '))}</article>`}).join('')}</div>`;
 if(id==='I02')return dashCards(ts,iconsById[id]);
 if(['R02','I03'].includes(id))return pairedCards(ts,iconsById[id],id==='R02'?['petri','microscope','lab']:[]);
 ...
 if(id==='C05')return `<div class="vision-grid">${[0,2].map((i,j)=>`<article>${icon(j?'target':'globe')}<span class="visual-label">${j?'OUR MISSION':'OUR VISION'}</span><h3>${e(ts[i])}</h3>${text(ts[i+1])}</article>`).join('')}</div>`;
 if(id==='C06')return `<div class="company-research-grid">${[0,1].map(i=>`<article>${photo(i?'petri':'microscope',i?'배양 소재의 가능성':'연구와 관찰')}<h3>${e(ts[i*2])}</h3>${text(ts[i*2+1])}</article>`).join('')}</div>`;
 if(id==='C07')return `<dl class="company-facts">${ts.map(t=>{const [k,...v]=t.split(':');return `<div><dt>${e(k)}</dt><dd>${e(v.join(':').trim())}</dd></div>`}).join('')}</dl>`;
 if(id==='CH02')return `<div class="history-layout"><aside>${photo('mycelium','자연에서 연구로')}<strong>2015—2026</strong><span>연구와 사업의 발자취</span></aside><ol class="history-line">${Array.from({length:Math.ceil(ts.length/2)},(_,i)=>{const [year,...t]=(ts[i*2]||'').split(' | ');return `<li><span class="timeline-dot"></span><strong>${e(year)}</strong><div><h3>${e(t.join(' | '))}</h3>${text(ts[i*2+1]||'')}</div></li>`}).join('')}</ol></div>`;
 if(['CS02','CS03','CS04','CS05'].includes(id)){const photos={CS02:'mycelium',CS03:'culture',CS04:'petri',CS05:'nature'};return `<div class="editorial-split ${id==='CS03'?'reverse':''}">${photo(photos[id],{CS02:'자연의 순환',CS03:'곡물배지와 배양 소재',CS04:'배양 환경의 관찰',CS05:'일상의 작은 실천'}[id])}<div>${icon(id==='CS03'?'grain':id==='CS04'?'settings':'leaf')}${full()}</div></div>`;}
 ...
 if(id==='R04')return `<div class="evidence-links">${ts.map((t,i)=>{const [a,b]=t.split(' — ');const urls=['/rnd/gmk/','/rnd/evidence/','/rnd/patents/','/business/development/#quality-production'];return `<a href="${urls[i]}">${icon(['culture','book','shield','settings'][i])}<div><h3>${e(a)}</h3>${text(b||'')}</div><span aria-hidden="true">↗</span></a>`}).join('')}</div>`;
 ...
 if(id==='RG03')return `<div class="diagram-heading">${ts[0]?`<h3>${e(ts[0])}</h3>`:''}</div>${infographic('culture')}<div class="diagram-notes">${ts.slice(1).map(text).join('')}</div>`;
 if(id==='RG04')return `<div class="editorial-split"><div>${full()}</div>${photo('powder','원료의 형태와 활용')}</div>`;
 if(id==='RE02'){
 ...
 if(id==='RP02')return dashCards(ts.filter(t=>t.includes(' — ')),iconsById.RP02)+ts.filter(t=>!t.includes(' — ')).map(text).join('');
 if(['B02','B03','B04','B05'].includes(id)){const ims={B02:'powder',B03:'lab',B04:'gift',B05:'science'};const icons={B02:'network',B03:'flask',B04:'leaf',B05:'globe'};return `<div class="editorial-split ${id==='B03'||id==='B05'?'reverse':''}">${photo(ims[id],{B02:'소재에서 시작되는 협력',B03:'제품화의 가능성',B04:'도두On 소비자 제품',B05:'새로운 시장으로'}[id])}<div>${icon(icons[id])}${full()}</div></div>${id==='B03'?infographic('business'):''}`;}
 ...
 if(id==='BD02')return `<ol class="development-steps">${ts.map((t,i)=>{const [a,b]=t.split(' — ');return `<li><span class="step-number">0${i+1}</span>${icon(['light','flask','search','document','partners','box'][i])}<h3>${e(a)}</h3>${text(b)}</li>`}).join('')}</ol>`;
 ...
 if(id==='I04'){
 return `<div class="growth-roadmap">${Array.from({length:Math.ceil(ts.length/2)},(_,i)=>`<article><div class="roadmap-mark">${icon(['flask','network','globe'][i])}<span>0${i+1}</span></div><div><h3>${e(ts[i*2])}</h3>${text(ts[i*2+1]||'')}</div></article>`).join('')}</div>`;
 }
