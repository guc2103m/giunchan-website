...
export const sortPress=posts=>posts.map((p,i)=>({...p,_order:i})).sort((a,b)=>a.publishedAt.localeCompare(b.publishedAt)||a._order-b._order);
...
export function newsroomContent(posts){
 ...
 return `<div class="newsroom container"><header class="newsroom-heading"><div class="eyebrow">NEWSROOM</div><h1>뉴스룸</h1><p>${newsroomDescription}</p><span class="newsroom-count">전체 ${posts.length}건 · 과거부터 최신순</span></header>${years.map(year=>`<section class="newsroom-year" aria-labelledby="year-${year}"><h2 id="year-${year}">${year}</h2><div>${sorted.filter(p=>p.publishedAt.startsWith(year)).map(p=>`<article class="newsroom-row"><div class="newsroom-meta"><time datetime="${e(p.publishedAt)}">${e(p.publishedAt.replaceAll('-','.'))}</time><span>${e(p.publisher)}</span></div><div class="newsroom-copy"><h3><a href="${pressPath(p)}">${e(articleTitle(p))}</a></h3><p>${e(intro(p))}</p><a class="text-link" href="${pressPath(p)}" aria-label="${e(articleTitle(p))} 자세히 보기">자세히 보기 →</a></div></article>`).join('')}</div></section>`).join('')}</div>`;
}
