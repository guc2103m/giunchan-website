...
const origin='https://giunchan-website.vercel.app';
...
export function insightsContent(researchCards){return `<section class="section" data-content-id="N01"><div class="container"><section id="research-panel" aria-labelledby="research"><h2 id="research" class="research-heading">연구자료</h2><div class="insight-grid" id="article-list">${researchCards}</div><nav class="research-pagination" aria-label="연구자료 페이지" hidden></nav><p class="research-page-status" aria-live="polite"></p></section></div></section>`;}
export function newsroomContent(posts){
 const sorted=sortPress(posts),years=[...new Set(sorted.map(p=>p.publishedAt.slice(0,4)))];
 return `<div class="newsroom container"><header class="newsroom-heading"><div class="eyebrow">NEWSROOM</div><h1>뉴스룸</h1><p>${newsroomDescription}</p><span class="newsroom-count">전체 ${posts.length}건 · 최신순</span></header>${years.map(year=>`<section class="newsroom-year" aria-labelledby="year-${year}"><h2 id="year-${year}">${year}</h2><div>${sorted.filter(p=>p.publishedAt.startsWith(year)).map(p=>`<article class="newsroom-row"><a class="newsroom-thumbnail" href="${pressPath(p)}" aria-label="${e(articleTitle(p))} 자세히 보기"><img src="/assets/logo.png" alt="" width="180" height="60" loading="lazy" decoding="async"></a><div class="newsroom-meta"><time datetime="${e(p.publishedAt)}">${e(p.publishedAt.replaceAll('-','.'))}</time><span>${e(p.publisher)}</span></div><div class="newsroom-copy"><h3><a href="${pressPath(p)}">${e(articleTitle(p))}</a></h3><p>${e(intro(p))}</p><a class="text-link" href="${pressPath(p)}" aria-label="${e(articleTitle(p))} 자세히 보기">자세히 보기 →</a></div></article>`).join('')}</div></section>`).join('')}</div>`;
}
...
export function pressMetadata(p){return `<link rel="canonical" href="${origin+pressPath(p)}">`;}
