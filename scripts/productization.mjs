...
const forms = new Set(['분말스틱', '액상차', '침출차', '분말', '젤리', '캡슐', '진액', '기타 고형제품']);
...
export function productizationPortfolio(products) {
  ...
  return `<section id="gmk-productization" class="article-section visual-section" aria-labelledby="gmk-productization-heading">
    ...
    <div class="gmk-case-grid" id="gmk-case-grid">${products.map((product, i) => `<figure class="gmk-case"${i >= 12 ? ' hidden data-additional-case' : ''}><div class="gmk-case-frame"><img src="${esc(product.src)}" alt="${esc(product.form)} 제형 제품화 사례" width="${product.width}" height="${product.height}" loading="lazy" decoding="async"></div><figcaption>${esc(product.form)}</figcaption></figure>`).join('')}</div>
