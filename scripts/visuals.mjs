...
export function enhanceSection(id,blocks,render){
 ...
 if(['B02','B03','B04','B05'].includes(id)){const ims={B02:'powder',B03:'product-development-support.png',B04:'gift',B05:'new-market.jpg'};const icons={B02:'network',B03:'flask',B04:'leaf',B05:'globe'};return `<div class="editorial-split ${id==='B03'||id==='B05'?'reverse':''}">${photo(ims[id],{B02:'소재에서 시작되는 협력',B03:'제품화의 가능성',B04:'도두On 소비자 제품',B05:'새로운 시장으로'}[id])}<div>${icon(icons[id])}${full()}</div></div>${id==='B03'?infographic('business'):''}`;}
