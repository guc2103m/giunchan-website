...
export function enhanceSection(id,blocks,render){
 ...
 if(id==='C04')return `<div class="ceo-layout">${img('company-ceo','기운찬 대표자','ceo-photo')}<div class="quote-panel"><div class="ceo-message-copy">${full()}</div><span class="quote-credit">GIUNCHAN · CEO MESSAGE</span></div></div>`;
 ...
 if(id==='RE02'){
  ...
  return `<div class="publication-intro">${ts.slice(0,start).map(text).join('')}${photo('microscope','과학적 근거를 축적하는 연구')}</div><div class="publication-grid">${bundles.map((a,i)=>`<article class="publication"><div class="publication-top"><strong>${e(a[1]?.slice(0,4)||'')}</strong><img class="publication-paper-icon" src="/assets/research-paper-${a[1]?.startsWith('2025')?'blue':'purple'}.png" width="48" height="48" alt="" aria-hidden="true" loading="lazy"></div><span class="paper-category">${e(a[0])}</span>${a.slice(1).map(t=>t.startsWith('논문명:')?`<h3 lang="en">${e(t.slice(4).trim())}</h3>`:t.startsWith('논문 원문:')?`<a class="text-link" target="_blank" rel="noopener noreferrer" href="${e(t.split(' ').at(-1))}">논문 원문 보기 ↗</a>`:text(t)).join('')}</article>`).join('')}</div>${tail.map(text).join('')}`;
 }
 if(id==='RE03')return `<div class="safety-panel"><img class="research-safety-icon" src="/assets/research-safety-test.png" width="96" height="96" alt="" aria-hidden="true" loading="lazy"><div>${full()}<div class="topic-chips"><span>반복투여 독성</span><span>단회투여 독성</span><span>유전독성</span></div></div></div>`;
 if(id==='RP01')return `<div class="patent-numbers"><div><img class="patent-count-icon" src="/assets/patent-count-korea.png" width="64" height="64" alt="" aria-hidden="true" loading="lazy"><span>국내 등록 특허</span><strong>9<small>건</small></strong></div><div><img class="patent-count-icon" src="/assets/patent-count-us.png" width="64" height="64" alt="" aria-hidden="true" loading="lazy"><span>미국 등록 특허</span><strong>1<small>건</small></strong></div></div>${full()}`;
 ...
 if(id==='BD03')return `<div class="editorial-split"><div>${full()}</div><div class="partnership-graphic partnership-logos"><div class="partner-owner"><div class="partner-owner-logo"><img src="/assets/partner-giunchan.png" alt="기운찬" loading="lazy"></div><span>소재·개발 협력</span></div><span class="partnership-symbol" aria-hidden="true">+</span><div class="partner-manufacturers"><div class="partner-logo-grid"><div class="partner-logo-cell"><img src="/assets/partner-vicell.png" alt="바이셀" loading="lazy"></div><div class="partner-logo-cell"><img src="/assets/partner-wooree-bio.png" alt="우리바이오" loading="lazy"></div><div class="partner-logo-cell"><img src="/assets/partner-jtp.png" alt="JTP" loading="lazy"></div><div class="partner-logo-cell"><img src="/assets/partner-kwangdong-health-bio.png" alt="광동헬스바이오" loading="lazy"></div><div class="partner-logo-cell"><img src="/assets/partner-o2bio.png" alt="오투바이오" loading="lazy"></div><div class="partner-logo-cell"><img src="/assets/partner-dawon-bio.png" alt="다원바이오" loading="lazy"></div></div><span>품목별 제조 협력</span></div></div></div>`;
 if(id==='I06')return `<div class="cta-copy"><span class="ir-bottom-icon ir-partner-icon"><img src="/assets/ir-partner-icon.png" alt="파트너 협력을 나타내는 악수 아이콘" width="1280" height="1280" loading="lazy"></span><div>${full()}</div></div>`;
 if(id==='BD04')return `<div class="cta-copy">${icon(id==='BD04'?'light':'partners')}<div>${full()}</div></div>`;
 ...
 if(id==='I05')return `<div class="quality-panel"><span class="ir-bottom-icon"><img src="/assets/ir-stage-icon.png" alt="단계별 검토를 나타내는 계단과 체크 아이콘" width="1420" height="1100" loading="lazy"></span><div>${full()}</div></div>`;
